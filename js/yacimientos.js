//Situar los marcadores, añadirlos al mapa y definir el texto a mostrar por el popup.
        ...
        var sidebar_pita = L.control.sidebar('sidebar_pita', {
            closeButton: true,
            position: 'left'
        });
        map.addControl(sidebar_pita);
        ...
        var sidebar_canovas = L.control.sidebar('sidebar_canovas', {
            closeButton: true,
            position: 'left'
        });
        map.addControl(sidebar_canovas);

        var sidebar_realLuengo = L.control.sidebar('sidebar_realLuengo', {
            closeButton: true,
            position: 'left'
        });
        map.addControl(sidebar_realLuengo);
        ...
        var marker = L.marker([43.455897, -8.226421]).addTo(map).on('click', function () {
            sidebar_pita.toggle();
        });
        ...
        var marker = L.marker([43.369154, -8.393732]).addTo(map).on('click', function () {
            sidebar_canovas.toggle();
        });

        var marker = L.marker([43.370328, -8.399280]).addTo(map).on('click', function () {
            sidebar_realLuengo.toggle();
        });
